Hi, I'm Amiya Pradhan 👋

Cloud security practitioner moving into AI security, analysing how AI systems break, how attackers exploit them, and how to defend them. Based in Sydney, Australia.

What I'm working on:
- In the [AI Security Fundamentals cohort](https://aisecurityfundamentals.com)
  with Mileva Security Labs (May–July 2026) - weekly write-ups in
  [ai-security-fundamentals-cohort-L1](https://github.com/amiyapradhan/ai-security-fundamentals-cohort-L1)
- Working through OWASP LLM Top 10 and MITRE ATLAS, building a
  [personal AI security learning log](https://github.com/amiyapradhan/ai-security-learning)
- Currently learning: AI security, MCP attack surfaces,
  and AI red teaming methodology

Highlights:
- [ai-security-learning](https://github.com/amiyapradhan/ai-security-learning) — evolving, topic-by-topic
  notes on frameworks, attacks, labs, and case studies
- [ai-security-fundamentals-cohort-L1](https://github.com/amiyapradhan/ai-security-fundamentals-cohort-L1) —
  week-by-week log of the cohort: my notes, my own code, my own diagrams
- [aisf-capstone-2026](https://github.com/amiyapradhan/aisf-capstone-2026) — end-to-end AI system
  security assessment (in progress)

How to reach me
- LinkedIn: [linkedin.com/in/amiyapradhan](https://linkedin.com/in/amiyapradhan)
- Email: [amiyapradhan@pm.me](mailto:amiyapradhan@pm.me)

